...
Some ACTIONS support additional ARGS.\n"))
  ...
  (display (G_ "
      --keep-host-uid-gid
                         for containers, run with the host's UID and GID
                         for correct access on shared/exposed files"))
  (newline)
  (display (G_ "
         ...
         (option '("expose") #t #f
                 (lambda (opt name arg result)
                   (alist-cons 'file-system-mapping
                               ...
                               result)))
         (option '("keep-host-uid-gid") #f #f
                 (lambda (opt name arg result)
                   (alist-cons 'keep-host-uid-gid? #t result)))
...
immediately.  Return the exit status of the process in the container."
  ...
  (define uid (if keep-host-uid-gid? (getuid) 1000))
  (define gid (if keep-host-uid-gid? (getgid) 1000))
           ...
           (case action
             ...
             ((container)
              (mlet %store-monad ((status (spawn-home-container
                                           ...
                                           #:keep-host-uid-gid? keep-host-uid-gid?
                                           #:command
                                           container-command)))
        ...
        (run-with-store store
          (mbegin %store-monad
            ...
            (perform-action action home-environment
                            ...
                            #:keep-host-uid-gid? (assoc-ref opts 'keep-host-uid-gid?)
